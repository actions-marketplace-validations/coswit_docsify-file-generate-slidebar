...
function has_matching_files() {
    ...
    for file in "$dir_path"/*; do
        [[ ! -f "$file" ]] && continue
        [[ $(basename "$file") =~ $include ]] && return 0
    done
    
    # 检查子目录是否包含匹配文件
    for file in "$dir_path"/*; do
        [[ ! -d "$file" ]] && continue
        has_matching_files "$file" "$include" && return 0
    done
...
function read_dir() {
    ...
    # 检查当前目录或其子目录是否有匹配文件
    if ! has_matching_files "$dir_path" "$include"; then
        return  # 如果不匹配，直接返回，不显示此目录
    fi
    
    # 如果是根目录或目录包含匹配文件，显示目录名
    if [[ $is_root -eq 1 ]] || has_matching_files "$dir_path" "$include"; then
